...
<main class="container my-5">
  ...
  <section class="">
    <form action="./src/api/event_update.php?event_id=<?= htmlspecialchars($_GET["event_id"]) ?>&organizator_id=<?= htmlspecialchars($event['organizator_id']) ?>" method="POST" class="row mb-3 border rounded-3 px-3 py-5">
      ...
      <div class="row mb-3">
        <div class="col-sm-3 mb-4">
          <?php if ($_GET["event_id"] && $_GET["event_id"] != "new") : ?>
            <h4>Event ID : <span class="ms-2"><?= htmlspecialchars($event["event_id"]) ?></span></h4>
          <?php else : ?>
            <h4>New event</h4>
          <?php endif ?>
        </div>
        <div class="col-sm-9 mb-4">
          <div class="row mb-3">
            <h6>Organizator ID : <span class="ms-2"><?= htmlspecialchars($event["organizator_id"]) ?></span></h6>
          </div>
          <div class="row">
            <h6>Organizator Name : <span class="ms-2"><?= htmlspecialchars($event["organizator_name"]) ?></span></h6>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-sm-4 mb-4">
          ...
          <input type="date" class="form-control date-picker" name="date" value="<?= htmlspecialchars($event["date"]) ?>" required>
        </div>
        <div class="col-sm-2 mb-4">
          <label for="hour" class="form-label">Hour*</label>
          <input type="number" class="form-control date-picker" name="hour" value="<?= htmlspecialchars($event["hour"]) ?>" min="0" max="23" required>
        </div>
        <div class="col-sm-2 mb-4">
          <label for="min" class="form-label">Minutes*</label>
          <input type="number" class="form-control date-picker" name="min" value="<?= htmlspecialchars($event["min"]) ?>" min="0" max="59" required>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-sm-4 mb-4">
          ...
          <input type="date" class="form-control date-picker" name="register_deadline" value="<?= htmlspecialchars($event["register_deadline"]) ?>">
        </div>
        <div class="col-sm-4 mb-4">
      ...
      <div class="row mb-4">
        <div class="col">
          ...
          <textarea class="form-control" name="description" style="height: 150px" required><?= htmlspecialchars($event["description"]) ?></textarea>
        </div>
      </div>
      ...
      <div class="row mb-4">
        <div class="col">
          ...
          <textarea class="form-control" name="details" style="height: 350px" required><?= htmlspecialchars($event["details"]) ?></textarea>
        </div>
      </div>
      ...
      <div class="row">
        <div class="mb-4">
          ...
          <select class="form-select" name="location_id" aria-label="Select city" required>
            <?php $sel_location = htmlspecialchars($event['location_id']) ?>
            <option <?php if ($sel_location == NULL) echo ('selected') ?> value="NULL" disabled>City - Location name : Address</option>
            <?php foreach ($locations as $location) : ?>
              <option <?php if ($sel_location == $location["location_id"]) echo ('selected') ?> value="<?= htmlspecialchars($location["location_id"]) ?>">
